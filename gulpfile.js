...
class Router {
	constructor(defaults, projectSettings) {
		...
		this.srcSassPath = `${this.srcPath}/sass`;
...
gulp.task('watch', (cb) => {
	...
	let globSrcStyles = `${bs.router.srcSassPath}/styles.sass`;
...
gulp.task('develop', gulp.parallel('watch', () => {
	...
	compilePages(`${bs.router.srcMarkupPath}/*.pug`, `${bs.router.buildPath}`);
	compileStyles(`${bs.router.srcSassPath}/styles.sass`, `${bs.router.dstStylesPath}`);
...
gulp.task('images', () => {
	...
	return Promise.all([
		new Promise( (resolve, reject) => {
			gulp.src(`${bs.router.srcAssetsPath}/images/*.jpg`)
				.pipe(imagemin([
			    	imageminGuetzli({
				        quality: 85
				    })
				]))
				.pipe(gulp.dest(`${bs.router.dstAssetsPath}/images`))
				.on('end', resolve);
		}),
		new Promise( (resolve, reject) => {	
			gulp.src(`${bs.router.srcAssetsPath}/images/*.png`)
				.pipe(pngquant(			{
					quality: '65-80'
				}))
				.pipe(gulp.dest(`${bs.router.dstAssetsPath}/images`))
...
// gulp.task('bootstrap', () => {
// 	let bs = Bootstrapper.create();
// 	return gulp.src(`${bs.router.srcPath}/sass/index.sass`)
// 	    .pipe(sass().on('error', sass.logError))
